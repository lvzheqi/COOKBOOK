...
public class EpubOpf {
    ...
    public EpubOpf(List<Element> elements, List<EpubItem> items) {
        ...
        pack.addContent(setMetadata(pack));
    ...
    private Element setMetadata(Element pack) {
        Element metadata = new Element("metadata", pack.getNamespace());

        metadata.addContent(new Element("title", ns2).setAttribute("id", "title").addContent(ExportInfo.title));
        ...
        metadata.addContent(new Element("contributor", ns2).addContent(ExportInfo.firstName + " " + ExportInfo.lastName));
