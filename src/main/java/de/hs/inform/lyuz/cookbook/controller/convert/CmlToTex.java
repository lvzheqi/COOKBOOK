...
public class CmlToTex {
    ...
    private final LinkedHashMap<String, Cookml> cook;
    private String texString;
    private int anzpics = 0;
    private final String name;

    // Constructor
    public CmlToTex(LinkedHashMap<String, Cookml> list, String kochbuchname) {
        this.texString = "";
        this.cook = list;
        this.name = kochbuchname + "_";
        getTexFromList();
    }

    // get complete Tex output
    public String getTex() {
        return texString;
    }

    private void getTexFromList() {
        cook.keySet().stream().map((catakt) -> {
            texString += cat2Tex(catakt);
            return catakt;
        }).forEachOrdered((catakt) -> {
            int anzahl = 0;
            for (Recipe recakt : cook.get(catakt).getRecipe()) {
                texString += rec2Tex(recakt, anzahl);
                anzahl++;
            }
        });
    }
    ...
    private String cat2Tex(String cat) {
        ...
        cattex += "%----------------------------------------------------\n";
        cattex += "\\nopagebreak{ \n";
        cattex += "\\" + "section{" + translatecategory(cat) + "} \n\n";
        return cattex;
    }
    ...
    private String translatecategory(String cat) {
        ...
        if (cat.equals("Salat")) {
            ...
            texcat = "Suppen und Eintöpfe";
        } else if (cat.equals("Auflauf")) {
            texcat = "Aufläufe und Gratins";
        } else if (cat.equals("Fisch")) {
            texcat = "Fischgerichte";
        } else if (cat.equals("Fleisch")) {
            texcat = "Fleischgerichte";
        } else if (cat.equals("Geflügel")) {
            texcat = "Geflügelgerichte";
        } else if (cat.equals("Gemüse")) {
            texcat = "Kartoffel- und Gemüsegerichte";
        } else if (cat.equals("Vegetarisch")) {
            texcat = "Vegetarisches";
        } else if (cat.equals("Saucen")) {
            texcat = "Saucen, Marinaden und Dips";
        } else if (cat.equals("Nachtisch")) {
            texcat = "Süßes";
        } else if (cat.equals("Backen")) {
            texcat = "Kuchen und Gebäck";
        } else if (cat.startsWith("Drink")) {
            texcat = "Getränke";
        } else if (cat.startsWith("Pasteten")) {
            texcat = "Pasteten und Terrinen";
        }
        return texcat;
    }
    ...
    private String rec2Tex(Recipe recipe, int anzahl) {
        ...
        rectex += titletex + texString(indextex + parttex + pictex + preptex + remarktex + servetex + conttex
                + timetex + sourcetex);
        rectex += "} \n\n";
        return rectex;
    }
    ...
    private String serve2tex(Head head) {
        ...
        String type = head.getServingtype();
        if (type != null && !type.equals("")) {
            Integer qty = Integer.valueOf(head.getServingqty());
            if (qty > 0) {
                type = qty.toString() + " " + type;
            }
            if (!type.equals("")) {
                type = "{\\" + "bfseries Menge:} " + type + "\n\n";
            }
        }
        return type;
    }
    ...
    private String pic2tex(Head head) {
        String pictex = "\n\n";
        pictex = head.getPicbin().stream().map((pic) -> {
            anzpics++;
            return pic;
        }).map((pic) -> {
            String filename = name + ((Integer) anzpics).toString() + "."
                    + pic.getFormat().toLowerCase(Locale.getDefault());
            Utils.writePictoFile(pic.getValue(), "LATEX/image/" + filename);
            return filename;
        }).map((filename) -> "\\bild{" + "image/" + filename + "}\n\n").reduce(pictex, String::concat);
    ...
    private String ing2tex(Ingredient ing) {
        ...
        text = ing.getUnit();
        if (text != null) {
            ingtex += FormatHelper.formatUnit(text) + " ";
        }
